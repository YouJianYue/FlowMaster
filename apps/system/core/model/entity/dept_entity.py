...
from sqlalchemy import Column, String, BigInteger, Text, Boolean, Integer, Index
...
class DeptEntity(BaseEntity):
    ...
    parent_id: Optional[int] = Column(BigInteger, nullable=False, default=0, comment="上级部门ID")

    # 祖级列表 - 对应参考项目的ancestors字段，默认为空字符串
    ancestors: Optional[str] = Column(String(512), nullable=False, default='', comment="祖级列表")
    ...
    # role_depts = relationship("RoleDeptEntity", back_populates="dept", cascade="all, delete-orphan")
    ...
    def get_ancestor_ids(self) -> List[int]:
        ...
        if not self.ancestors:
            return []
        try:
            return [int(id_str) for id_str in self.ancestors.split(",") if id_str.strip() and id_str != '0']
