...
class MenuServiceImpl:
    ...
    async def create_menu(self, menu_req: MenuReq) -> MenuResp:
        ...
        async with DatabaseSession.get_session_context() as session:
            ...
            menu_entity = MenuEntity(
                ...
                icon=menu_req.icon,
                is_external=menu_req.is_external if menu_req.is_external is not None else False,
                is_cache=menu_req.is_cache if menu_req.is_cache is not None else False,
                is_hidden=menu_req.is_hidden if menu_req.is_hidden is not None else False,
    ...
    async def update_menu(self, menu_id: int, menu_req: MenuReq) -> MenuResp:
        ...
        async with DatabaseSession.get_session_context() as session:
            ...
            menu_entity.is_external = menu_req.is_external if menu_req.is_external is not None else False
            menu_entity.is_cache = menu_req.is_cache if menu_req.is_cache is not None else False
            menu_entity.is_hidden = menu_req.is_hidden if menu_req.is_hidden is not None else False
    ...
    def _convert_to_dict_tree(self, menu_tree: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        转换为字典树格式
