...
from apps.system.core.data.menu_initial_data import get_menu_data, build_menu_tree
...
class MenuService:
    ...
    async def get_user_route_tree(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        for menu in user_menus:
            if (
                menu.get("status") == 1
                and not menu.get("is_hidden", False)
                and menu.get("type") in [1, 2]
            ):  # 1=目录, 2=菜单
    ...
    async def build_menu_tree_with_permissions(
        self, user_id: int
    ) -> List[Dict[str, Any]]:
        """
        构建包含权限信息的菜单树
