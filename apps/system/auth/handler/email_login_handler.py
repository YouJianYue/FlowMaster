...
class EmailLoginHandler(AbstractLoginHandler):
    ...
    async def _get_user_by_email(self, email: str) -> 'UserEntity':
        """
        根据邮箱获取用户实体
    ...
    async def _log_login_failure(self, email: str, reason: str, extra_info: Dict[str, Any]):
        """记录登录失败日志"""
        # TODO: 实现登录失败日志记录
        print(f"邮箱登录失败 - 邮箱: {email}, 原因: {reason}, 额外信息: {extra_info}")
        pass